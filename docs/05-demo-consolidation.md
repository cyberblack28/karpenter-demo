...
## おさらい：Consolidation とは

Karpenter が継続的にクラスタを監視し、**無駄なノードを自動で整理する仕組み**です。3 パターンあります。

| # | パターン | 内容 |
| --- | --- | --- |
| ① | 空のノードを削除 | Pod が居なくなったノードを消す（**本デモで主に見えるのはこれ**） |
| ② | Pod を寄せて削除 | Pod を他ノードに退避できるなら、まとめてノードを消す |
| ③ | 小さいノードへ置換 | 過剰スペックのノードを、より小さく安いノードに交換 |

本デモの NodePool は `consolidationPolicy: WhenEmptyOrUnderutilized` なので **①②③すべて**が対象です。
詳しい解説は [00-overview.md](00-overview.md#consolidation統合とは) にあります。
